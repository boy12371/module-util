package io.swagger.util.common2.validation;
...
public class  Validation {
    ...
    private static Pattern idNum18 = Pattern.compile("^([1-9][0-9]{16}[0-9Xx])");
    private static Pattern mobile = Pattern.compile("^([1][34578][0-9]{9})$");
    private static Pattern zip = Pattern.compile("^[0-9]{6}$");
    
    private static Pattern usernamefirst = Pattern.compile("^[0-9]$");

    
    public static boolean isNum(String zipstr) {
        return usernamefirst.matcher(zipstr).matches();
    }
    ...
    private static String countCheckCode(String idno) {
        ...
        System.out.println("lastCode.substring(mod, mod+1).toUpperCase():"+
     ...
     * @return
     */
    public static boolean isValidIdNum(String idno) {
        ...
        if (!idNum18.matcher(idno).matches()) {
        ...
        if (!isDate(birthday)) {
            System.out.println("=============生日不对");
            return false;
        }

        //校验码检查
        if (idno.length() == 18) {
            if (!idno.substring(17, 18).toUpperCase().equals(countCheckCode(idno))) {
        ...
        if (family_type > 0) {
            ...
            for (int i = 1; i <= family_member_count; i++) {
                ...
                if (map.get("id_type_" + i).equals("1") || map.get("id_type_" + i).equals("31")) {
                    if (!isValidIdNum(map.get("id_no_" + i))) {
    ...
    private static boolean checkWrc(Map<String, String> map, List<String> messages) {
        ...
        if (map.get("wrc_id_type").equals("1") && isValidIdNum(map.get("wrc_id_no"))) {
            messages.add("请录入办理使用证件号码！");
            return false;
        }
        return true;
    ...
    private static boolean checkZhubaoYaoHao(Map<String, String> map, List<String> messages, int type) {
        ...
        if (map.get("guarantee_applier_id_type").equals("1")
                && !isValidIdNum(map.get("guarantee_applier_id_no"))) {
